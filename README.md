...
**Версия документации:** v1.7 03.02.2026
...
- [docs/00_INDEX.md](docs/00_INDEX.md) — стабильный индекс якорных документов.
- Ключевые якоря: [01 Канон](docs/01_CANON.md), [02 Техспека](docs/02_TECH_SPEC.md).
...
Полные детали — в [docs/02 (раздел «Модули»)](docs/02_TECH_SPEC.md).
...
## Рабочий процесс с Codex (без IDE-ошибок)
1) Маленькие итерации: 1–2 файла UI за раз, без массовых перезаписей.
2) Перед изменениями — перечитать якоря 01–04 и guardrails (docs/08).
3) Не трогать стили/XAML без явной задачи; не переносить WPF-паттерны в Avalonia.
4) Обязательно проверять сборку и lint до коммита.
...
scripts/lint-avalonia.sh
```

Дополнительно (если установлен инструмент):
```bash
dotnet format
```

**Быстрый XAML lint (запрещённые токены):**
```bash
rg -n "SetterTargetType|ElementName='\\\"| Mode=|WrapPanel.*Spacing|TargetFrameworkAttribute" .
```
