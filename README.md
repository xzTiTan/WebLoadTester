...
**Playwright-браузеры:**
**Примечание по канону TO-BE:** якорный документ `docs/INDEX.md` фиксирует целевой путь браузеров как `{DataDirectory}/browsers`. В текущей реализации путь может отличаться; это известная дельта AS-IS и исправляется отдельным эпиком (Playwright contract).
