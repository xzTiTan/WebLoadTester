...
## Project snapshot
WebLoadTester — кроссплатформенное настольное приложение на .NET 8 + Avalonia для запуска 10 видов тестирования и диагностики Web-сайтов с отчётами и артефактами. Архитектура модульная (Core + Infrastructure + Modules + Presentation), UI построен по MVVM, результаты сохраняются в `runs/{RunId}/`. Источник истины — якорные документы в `docs/`.
