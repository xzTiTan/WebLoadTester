...
## 6. Журнал изменений
### v2.16 16.02.2026
- Исправлена ошибка компиляции CS0103 в `Presentation/ViewModels/MainWindowViewModel.cs`: в `StartAsync` восстановлено формирование локального `RunProfile` через `RunProfile.BuildProfileSnapshot(...)`, и этот объект используется для Telegram/preflight и создания `RunContext`.
