...
5a. [OK] Центральные секции не сворачиваются (Expander отсутствует).

## 2) Running locks

6. [OK] Tabs не блокируются во время `Running` (доступно переключение групп для просмотра).
...
14. [OK] После Add/Duplicate фокус переводится в первое доступное поле новой/дублированной строки (при сохранении стабильного selection + scroll).
...
35. [OK] Если guard блокирует переход (IsDirty), prompt видим и доступен; при появлении prompt UI скроллится вверх.

36. [OK] Смена верхней вкладки сама по себе не помечает конфиг dirty (`RunProfile.IsUiFamily` — UI-only переключатель семейства).
...
- **BUG:** не обнаружены.
- **RISK:** не обнаружены.
- Для закрытия ранее отмеченного риска добавлена точечная правка фокуса после Add/Duplicate в RowListEditor (без изменения доменной логики).
