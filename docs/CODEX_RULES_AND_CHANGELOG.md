...
**Версия:** v3.0 16.02.2026  
**Канон проекта:** см. `docs/INDEX.md` (единый исходный документ).  
**Цель файла:** описать правила работы агента (Codex) с репозиторием + вести журнал изменений.

---

## 0. Как работать агенту (обязательный порядок)

1) Прочитать `docs/INDEX.md` целиком и считать его **TO‑BE** источником правды.  
2) Затем прочитать этот файл (`docs/CODEX_RULES_AND_CHANGELOG.md`).  
3) Только потом менять код/доки.

Если обнаружены расхождения AS‑IS ↔ TO‑BE — фиксировать в PR как «Дельта внедрения» (см. §16 в INDEX).
