...
- Добавлен `InputValueGuard` для единых правил восстановления/ограничения int/enum/text значений в RunProfile и settings row-editors.
- Убраны лишние массовые нотификации в `Workspace/TestCaseViewModel` (точечные `PropertyChanged`) для снижения потери фокуса и лишних перерисовок.
- В `RowListEditorView` убран автоматический `ScrollIntoView` на каждое изменение выбора; автоскролл сохранён только при явном фокус-запросе (добавление/дублирование).
- Обновлены версии `README.md`, `AGENTS.md`, `docs/INDEX.md`.

## v3.51 09.03.2026
- UX Fix Pack: устранён блокирующий NavGuard по dirty при смене вкладки/модуля — теперь несохранённые изменения мягко отменяются с восстановлением последнего сохранённого состояния (или default при его отсутствии), без тупиков навигации.
- UX Fix Pack: снижены перерисовки редакторов списков (`RowListEditorViewModel.SetItems`) — при вводе в строках больше не пересобирается Items при неизменном наборе ссылок, что убирает потерю фокуса/прыжки скролла.
- UX Fix Pack: validate-on-start/save закреплён в поведении запуска — навигация больше не блокируется из-за промежуточных ошибок ввода.
- UX Fix Pack: в workspace добавлены явные секции «Что это за тест», «Настройки ...», «Параметры запуска», «Конфигурация», «Старт/Стоп/Статус/Результаты»; усилена индикация состояния прогона и быстрых путей к артефактам.
- Логирование улучшено для демонстрации: явные INFO о старте/завершении прогона и путях к результатам.
...
- `MainWindowViewModel.OnSelectedTabIndexChanged` больше не применяет dirty-guard к смене вкладки во время Running (устранены ложные блокировки/мигание).
- Обновлены `docs/03_UI_CANON.md`, `docs/INDEX.md` и `docs/Каркас.md` для устранения противоречий по Tab lock.
- Обновлены версии `README.md` и `AGENTS.md`.
...
## 09.03.2026 — v3.45
- Проект переориентирован под учебную модель АИС тестирования и диагностики Web-сайтов (10 видов тестирования в публичной номенклатуре).
- Обновлены публичные имена и описания модулей в `ModuleCatalog` и модулях без risky mass rename внутренних `moduleId`.
- Модуль `ui.timing` расширен до MVP тестирования совместимости (browser/viewport/user-agent per profile).
- Улучшен `report.html`: добавлены сводка, критичность, проблемы, рекомендации и простая диаграмма успешности.
- Зафиксировано сохранение совместимости `report.json`/repeat-run/top-level полей.


## 09.03.2026 — v3.46
- Проведён пост-миграционный аудит публичных текстов и UI-заголовков: устранены остаточные старые названия в основных экранах/формах модулей.
- В `ui.scenario` реализован устойчивый MVP baseline compare (последний успешный прогон той же конфигурации шагов) с итоговым результатом `Регрессионное сравнение`.
- В `report.html` добавлен отдельный блок отображения регрессионного сравнения.
- Для дымового/безопасности/общих UI-подписей приведены публичные русские формулировки в единый стиль учебной модели АИС.


## 09.03.2026 — v3.47
- Точечно исправлены ошибки сборки без изменения архитектуры и `moduleId`:
  - `HtmlReportWriter` больше не обращается к `DetailsJson` через `ResultBase`, добавлен безопасный helper по типам результатов.
  - `ui.scenario`: `TestStatus.Skipped` заменён на `TestStatus.Partial` в MVP baseline compare.
  - `ui.timing`: вызов `NewPageAsync` приведён к поддерживаемой сигнатуре; viewport задаётся отдельным вызовом `SetViewportSizeAsync`.
- Подтверждён статический аудит совместимости `report.json`/repeat-run (top-level контракт не изменялся).
