...
## 3. Guardrails для Codex (UI и сборка)
- Запрещены WPF-атрибуты и синтаксис XAML 2006 (например, `SetterTargetType`). Использовать только Avalonia XAML.
- Любой изменяемый XAML обязан компилироваться Avalonia XAML compiler:
  - Указывать `x:DataType` на root View и на каждый `DataTemplate`.
  - Если используешь `ElementName=Root`, обязателен `x:Name="Root"`.
  - Внутри `DataTemplate` биндинги только типизированные.
  - Для доступа к VM из вложенных контролов использовать `RelativeSource` или `#Root`.
- Никаких TextBlock-оверлеев как placeholder. Для TextBox использовать `Watermark`.
- Не добавлять ручные assembly-атрибуты (AssemblyInfo) при включённой автогенерации.
- Изменения должны собираться через `dotnet clean && dotnet build` без AVLN/MC-ошибок и CS0579.

## 4. Политика веток/PR
- Ветка: `codex/<topic>-<short>`
- PR summary: что сделано, что не сделано, тестирование.

## 5. Версионирование якорных файлов
- Любая правка якорного файла → поднять версию и дату в шапке файла.
- Добавить запись в §5 «Журнал изменений».

## 6. Журнал изменений
### v1.7 10.03.2026
- Введён guardrails-раздел для Codex (Avalonia XAML, типизация, Watermark, сборка).
- Обновлён канон UI: закреплённые ряды ModuleWorkspace, единая конфигурация и параметры запуска.
- Лог выполнения вынесен в общий Log Drawer.
