...
- Во время Running: **Tabs и LeftNav блокируются** (см. раздел 2).
...
4) **Help / Shortcuts**
   - F5 Start, Shift+F5 Stop, Ctrl+L лог, Ctrl+S сохранить
...
- Во время Running: Tabs и LeftNav **disabled**.
...
- Tabs: UI/HTTP/NetSec/Runs.
- Во время Running: Tabs должны быть disabled (binding на IsRunning).
...
5) Смена модуля/вкладки во время Running запрещена.
...
- Running блокирует Tabs и LeftNav.
...
3) Нажать Start → во время Running попытаться сменить вкладку/модуль → должно быть запрещено.
