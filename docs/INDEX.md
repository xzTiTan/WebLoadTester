...
**Актуально для:** состояния репозитория (единый TO-BE канон).
...
---
Дополнительные якорные документы (детализация TO-BE):
- Архитектура и данные: `docs/02_ARCHITECTURE_AND_DATA.md`
- UI-канон: `docs/03_UI_CANON.md`
- План тестирования и трассируемость: `docs/05_TEST_PLAN_AND_TRACEABILITY.md`
- Правила Codex и журнал изменений: `docs/CODEX_RULES_AND_CHANGELOG.md`

---
Дополнительная UI/UX спецификация минимального каркаса:
- UI/UX Spec (Minimal UI): `docs/Каркас.md`
...
* **10 видов тестирования (публичная модель), внутренне сохранены 3 семейства модулей для совместимости и отражены в UI как 4 вкладки верхнего уровня:
...
Правило UX: вкладки всегда доступны, включая Running (режим просмотра); при этом смена модуля и редактирование/сохранение конфигурации во время Running блокируются.
...
| ---------------- | -------------------------- | --------- | ----------------------------- |
| ui.scenario      | 3. Регрессионное тестирование           | UI        | шаги в браузере и сравнение с baseline |
| ui.snapshot      | 4. Интерфейсное тестирование            | UI        | скриншоты URL/элементов                |
| ui.timing        | 5. Тестирование совместимости           | UI        | профили browser/viewport/user-agent    |
| http.functional  | 2. Функциональное тестирование          | HTTP      | проверки условий                       |
| http.performance | 6. Тестирование производительности      | HTTP      | агрегаты latency/ошибок       |
| http.assets      | 10. Тестирование ресурсов Web-сайта     | HTTP      | проверка статических ресурсов          |
| net.diagnostics  | 9. Диагностическое тестирование         | NetSec    | безопасная диагностика                 |
| net.availability | 8. Тестирование доступности             | NetSec    | атомарная проверка            |
| net.security     | 7. Тестирование безопасности            | NetSec    | заголовки/redirect            |
| net.preflight    | 1. Дымовое тестирование                 | NetSec    | быстрый предстартовый чек              |
...
#### 8.3 ui.scenario — Регрессионное тестирование
...
#### 8.4 ui.snapshot — Интерфейсное тестирование
...
#### 8.5 ui.timing — Тестирование совместимости
...
#### 8.2 http.functional — Функциональное тестирование
...
#### 8.6 http.performance — Тестирование производительности
...
2. Добавляет endpoints* (как в функциональном тестировании). Поле `ExpectedStatusCode` **не обязательно**, но если задано — используется как проверка.
3. Настраивает профиль запуска (`Parallelism` + Iterations/Duration + `PauseBetweenIterationsMs`).
4. Нажимает **Старт**.
...
#### 8.10 http.assets — Тестирование ресурсов Web-сайта
...
#### 8.9 net.diagnostics — Диагностическое тестирование
...
#### 8.8 net.availability — Тестирование доступности
...
#### 8.7 net.security — Тестирование безопасности
...
#### 8.1 net.preflight — Дымовое тестирование
...
> Статус на 17.02.2026: ✅ Prompt 1–13 внедрены в код (сборка зелёная).
...
✅ 5) UI‑артефакты по воркерам: `profiles/w{WorkerId}`, `screenshots/w{WorkerId}`. *(внедрено Prompt 13: worker-scoped folders + snapshots профиля/настроек)*
...
✅ 13) Валидация: единый стандарт отображения ошибок (LostFocus + на Старт/Сохранить) и блокировок. *(внедрено Prompt 12A/12B: touched/submit + scroll к первой ошибке)*
14) Починить текущую ошибку спецификации (AS‑IS): смешение текста интерфейсного и диагностического модулей в старых ветках — в TO‑BE секции разведены.
...
* **Notes:** ссылки на разделы INDEX (например §6.6, §8.2).
...
* 17.02.2026 — v3.27 — Prompt 13: UI-модули сохраняют worker-scoped артефакты в `profiles/w{WorkerId}` и `screenshots/w{WorkerId}/it{Iteration}`; добавлены snapshots `profile.json` и `moduleSettings.json` на воркер и их регистрация в artifacts.
* 17.02.2026 — v3.26 — Prompt 12B: при блокировке Старт/Сохранить добавлен детерминированный переход к первой ошибке (scroll + focus), включая ключи таблиц/списков (`table.*`, `list.endpoints`).
* 17.02.2026 — v3.25 — Prompt 12A: введён единый UX валидации по touched/submit (показываем ошибки после LostFocus или Submit), добавлены summary-баннеры для конфигурации/настроек модуля/профиля запуска и блокировки Start/Save по ошибкам.
...
- Регрессионный модуль (`ui.scenario`) выполняет MVP baseline compare с последним успешным прогоном той же конфигурации шагов; при отсутствии baseline в `report.html` и результатах фиксируется явное сообщение.
- Модуль совместимости (`ui.timing`) использует отдельный экран/VM и сохраняет профиль совместимости (browser/viewport/user-agent/headless) в результаты для отчётной матрицы.
