...
* Реализация: `WrapPanel` **с `ItemSpacing` и `LineSpacing`** (не использовать несуществующий `Spacing` у WrapPanel).
...
* Любая «строка кнопок» (CRUD, RunPanel actions, TableToolbar) делается через `WrapPanel` с:

  * `ItemSpacing=8`, `LineSpacing=8`.
* Кнопки имеют `MinWidth=ButtonMinWidth`, текст **не переносится**.
...
3. В панели деталей выбранного endpoint настраивает проверки (минимум):

   * `ExpectedStatusCode`* (например 200)
   * опционально: `MaxLatencyMs`, `RequiredHeaders`, `BodyContains`, `JsonFieldEquals` (простые проверки; без regex/jsonpath в MVP)
4. Нажимает **Старт**.
...
* RunItems по endpoints (status code, latency, нарушенные условия)
...
2. Добавляет endpoints* (как в B1, но без обязательных assert’ов кроме status по желанию).
3. Настраивает профиль запуска (Parallelism + Iterations/Duration + PauseBetweenIterationsMs).
4. Нажимает **Старт**.

Результат:

* агрегаты latency/ошибок по endpoints + общий summary
...
2. Реализовать модель `Parallelism = N workers` + `WorkerId/Iteration` в контексте и в отчётах.
3. Реализовать Stop (soft) отдельно от Cancel (hard) и корректную финализацию статусов.
...
6. `net.availability`: убрать legacy IntervalSeconds и циклы; использовать RunProfile Duration/Iterations + PauseBetweenIterationsMs.
7. Telegram: settings.json, тестовое сообщение, уведомления onStart/onFinish/onError с rate limit.
8. `http.functional`: List+Details (устранить ошибки биндинга/MC3073), валидации endpoint’ов.
...
12. Везде, где есть таблица: внедрить TableToolbar + единые команды Add/Delete/Duplicate (+ Up/Down только там, где порядок важен).
...
### 18.2. Стратегия: серия из 7 промптов (рекомендуемый план)

Промпты должны быть «узкими»: чёткая цель, чёткий список файлов, чёткие acceptance criteria.

**Prompt 1 — Стабилизация сборки и устранение текущих UI‑ошибок**

* Цель: исправить ошибки XAML/стилей/биндингов, привести компиляцию к зелёному состоянию.
* Scope: `Presentation/Styles`, проблемные Views, binding‑ошибки (например MC3073/AVLN…).
* Acceptance: `dotnet build` проходит; UI запускается.

**Prompt 2 — Контракт отчётов и run folder (report.json, artifacts, moduleSettings snapshot)**

* Цель: реализовать обязательный контракт `report.json` (§9.1) + запись артефактов в `runs/{RunId}/`.
* Scope: `Core/Reporting`, `Infrastructure/Artifacts`, `TestRuns` snapshot, генерация report.json.
* Acceptance: для любого модуля создаётся `runs/{RunId}/report.json` со всеми обязательными полями.

**Prompt 3 — Оркестратор: parallelism/workers, iterations semantics, pause, stop/cancel**

* Цель: привести `RunOrchestrator` к §5 (воркеры, итерации, Duration, PauseBetweenIterationsMs, Stop vs Cancel).
* Scope: `Core/Orchestrator`, `RunContext`, статусы.
* Acceptance: корректные статусы, корректные `workerId/iteration` в RunItems, pause работает.

**Prompt 4 — UI Layout рефактор (анти‑наложения) по §6.2/§6.6/§17**

* Цель: внедрить `FieldBlock`, переносимые button panels (WrapPanel ItemSpacing/LineSpacing), ModuleWorkspace каркас.
* Scope: `Presentation/Views/*`, `Styles`, общие компоненты.
* Acceptance: UI‑smoke на `1024x720` проходит визуально; нет перекрытий/ухода за границы.

**Prompt 5 — Модули UI (ui.scenario/ui.snapshot/ui.timing) до рабочего MVP**

* Цель: привести UI и валидацию к §8.A, внедрить TableToolbar, screenshot policy семантику.
* Scope: соответствующие `Modules/*` и `Views/*`.
* Acceptance: запускаются, пишут report.json, создают скриншоты по правилам.

**Prompt 6 — Модули HTTP (functional/performance/assets) до рабочего MVP**

* Цель: List+Details для endpoints (functional/performance), DataGrid для assets, валидации.
* Scope: HTTP modules + views.
* Acceptance: все 3 модуля запускаются и пишут RunItems.

**Prompt 7 — NetSec модули + вкладка «Прогоны» (repeat/delete/open)**

* Цель: довести net.diagnostics/availability/security/preflight до §8.C и реализовать Runs tab по §6.4.
* Scope: net modules + runs viewmodels/views.
* Acceptance: repeat run заполняет форму из report.json; delete удаляет БД+папку.
...
* «Если в INDEX чего‑то нет — не добавляй; вместо этого оставь TODO‑комментарий в коде и отметь в `CODEX_RULES_AND_CHANGELOG.md` как вопрос/долг.»
