...
> Статус на 17.02.2026: ✅ Prompt 1–6 внедрены в код (сборка зелёная). Далее по плану: Prompt 7.
...
✅ 8) `http.functional/http.performance/http.assets`: List+Details и DataGrid‑паттерны, русская валидация, детерминированные RunItems (Endpoint/Asset), MVP assert’ы. *(внедрено Prompt 6; ожидает коммита/PR)*
...
**Prompt 6 — (DONE, требуется проверить/закоммитить в master перед Prompt 7) HTTP‑семейство (functional/performance/assets)**
...
**Prompt 7A — NetSec‑семейство (net.diagnostics / net.availability / net.security / net.preflight)**
