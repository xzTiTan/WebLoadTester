...
- Для `ui.scenario` добавлено человекочитаемое регрессионное сравнение с baseline (без изменения top-level контракта `report.json`).
- Для `ui.timing` профиль совместимости сериализуется в `items[].metrics/extra` через `DetailsJson` и отображается в HTML-отчёте.
