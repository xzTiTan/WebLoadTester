...
|---|---|---|---|---|
| FR-01 | RunId для каждого прогона + папка runs/<runId>/ | Все | `MainWindowViewModel`, `ArtifactStore`, `TestOrchestrator` | 1.1 |
| FR-02 | report.json формируется всегда | Все | `JsonReportWriter`, `ArtifactStore.SaveJsonAsync` | 1.2 |
...
| FR-08 | Режимы нагрузки ядра: Iterations/Duration | Все | `RunProfile.Mode`, логика в модулях | 2.x |
